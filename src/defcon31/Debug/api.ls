 ...
 183                     ; 92 void setup_serial(bool is_enabled,bool is_fast_baud_rate)
 183                     ; 93 {
 184                     	switch	.text
 185  000f               _setup_serial:
 187  000f 89            	pushw	x
 188       00000000      OFST:	set	0
 191                     ; 94 	if(is_enabled)
 193  0010 9e            	ld	a,xh
 194  0011 4d            	tnz	a
 195  0012 2747          	jreq	L17
 196                     ; 96 		GPIO_Init(GPIOD, GPIO_PIN_5, GPIO_MODE_OUT_PP_HIGH_FAST);
 198  0014 4bf0          	push	#240
 199  0016 4b20          	push	#32
 200  0018 ae500f        	ldw	x,#20495
 201  001b cd0000        	call	_GPIO_Init
 203  001e 85            	popw	x
 204                     ; 97 		GPIO_Init(GPIOD, GPIO_PIN_6, GPIO_MODE_IN_PU_NO_IT);
 ...
 216                     ; 99 		UART1_Init(is_fast_baud_rate?9600:1000000, UART1_WORDLENGTH_8D, UART1_STOPBITS_1, UART1_PARITY_NO, UART1_SYNCMODE_CLOCK_DISABLE, UART1_MODE_TXRX_ENABLE);
 ...
 278  0078 85            	popw	x
 279  0079 81            	ret
 306                     ; 110 bool is_application_valid()
 306                     ; 111 {
 307                     	switch	.text
 308  007a               _is_application_valid:
 312                     ; 112 	return !is_button_down(2) && !get_button_event(0,1);
 ...
 328  0090               L22:
 331  0090 81            	ret
 357                     ; 116 bool is_developer_valid()
 357                     ; 117 {
 358                     	switch	.text
 359  0091               _is_developer_valid:
 363                     ; 118 	return is_button_down(2) && !get_button_event(0,1);
 ...
 379  00a7               L03:
 382  00a7 81            	ret
 407                     ; 122 bool is_sleep_valid()
 407                     ; 123 {
 408                     	switch	.text
 409  00a8               _is_sleep_valid:
 413                     ; 124 	return !(get_button_event(0,0) || get_button_event(1,0) || get_button_event(0,1) || get_button_event(1,1));
 415  00a8 5f            	clrw	x
 ...
 439  00cf               L63:
 442  00cf 81            	ret
 468                     ; 127 void setup_main()
 468                     ; 128 {
 469                     	switch	.text
 470  00d0               _setup_main:
 474                     ; 129 	CLK->CKDIVR &= (u8)~(CLK_CKDIVR_HSIDIV);			// fhsi= fhsirc (HSIDIV= 0), run at 16 MHz
 476  00d0 c650c6        	ld	a,20678
 477  00d3 a4e7          	and	a,#231
 478  00d5 c750c6        	ld	20678,a
 479                     ; 131 	GPIO_Init(GPIOD, GPIO_PIN_1, GPIO_MODE_IN_PU_NO_IT);//SWIM input to choose between application and developer modes
 481  00d8 4b40          	push	#64
 482  00da 4b02          	push	#2
 483  00dc ae500f        	ldw	x,#20495
 484  00df cd0000        	call	_GPIO_Init
 486  00e2 85            	popw	x
 487                     ; 134 	TIM2->CCR1H=0;//this will always be zero based on application architecutre
 489  00e3 725f5311      	clr	21265
 490                     ; 135 	TIM2->PSCR= 6;// init divider register 16MHz/2^X
 492  00e7 3506530e      	mov	21262,#6
 493                     ; 136 	TIM2->ARRH= 0;// init auto reload register
 495  00eb 725f530f      	clr	21263
 496                     ; 137 	TIM2->ARRL= PWM_MAX_PERIOD;// init auto reload register
 498  00ef 35fa5310      	mov	21264,#250
 499                     ; 138 	TIM2->CR1|= TIM2_CR1_ARPE | TIM2_CR1_URS | TIM2_CR1_CEN;// enable timer
 501  00f3 c65300        	ld	a,21248
 502  00f6 aa85          	or	a,#133
 503  00f8 c75300        	ld	21248,a
 504                     ; 139 	TIM2->IER= TIM2_IER_UIE | TIM2_IER_CC1IE;// enable TIM2 interrupt
 506  00fb 35035303      	mov	21251,#3
 507                     ; 140 	enableInterrupts();
 510  00ff 9a            rim
 512                     ; 154 }
 516  0100 81            	ret
 540                     ; 156 u32 millis()
 ...
 601                     ; 164 }
 604  0110 81            	ret
 662                     ; 169 void update_buttons()
 662                     ; 170 {
 663                     	switch	.text
 664  0111               _update_buttons:
 666  0111 5208          	subw	sp,#8
 667       00000008      OFST:	set	8
 670                     ; 171 	bool is_any_down=0;
 672  0113 0f05          	clr	(OFST-3,sp)
 674                     ; 173 	u16 elapsed_pressed_ms=millis()-button_start_ms;
 ...
 693                     ; 174 	for(button_index=0;button_index<BUTTON_COUNT;button_index++)
 695  012e 0f08          	clr	(OFST+0,sp)
 697  0130               L112:
 698                     ; 176 		if(is_button_down(button_index))
 700  0130 7b08          	ld	a,(OFST+0,sp)
 701  0132 cd01dd        	call	_is_button_down
 703  0135 4d            	tnz	a
 704  0136 271b          	jreq	L712
 705                     ; 178 			if(!button_start_ms) button_start_ms=millis();//if button is down and haven't started a button press event, start it
 ...
 718  0148               L122:
 719                     ; 179 			set_debug(255);//only need to enable this when true.  Is automatically cleared every frame
 721  0148 a6ff          	ld	a,#255
 722  014a cd04b4        	call	_set_debug
 724                     ; 180 			is_any_down=1;
 726  014d a601          	ld	a,#1
 727  014f 6b05          	ld	(OFST-3,sp),a
 730  0151 2022          	jra	L322
 731  0153               L712:
 732                     ; 182 			if(elapsed_pressed_ms>BUTTON_LONG_PRESS_MS) button_pressed_event[button_index][1]=1;
 ...
 747  0165               L522:
 748                     ; 183 			else if(elapsed_pressed_ms>BUTTON_MINIMUM_PRESS_MS) button_pressed_event[button_index][0]=1;
 ...
 761  0175               L322:
 762                     ; 174 	for(button_index=0;button_index<BUTTON_COUNT;button_index++)
 764  0175 0c08          	inc	(OFST+0,sp)
 768  0177 7b08          	ld	a,(OFST+0,sp)
 769  0179 a102          	cp	a,#2
 770  017b 25b3          	jrult	L112
 771                     ; 187 	if(!is_any_down) button_start_ms=0;
 ...
 786  018d 81            	ret
 832                     ; 191 bool get_button_event(u8 button_index,bool is_long)
 832                     ; 192 { return button_pressed_event[button_index][is_long]; }
 833                     	switch	.text
 ...
 855  019c 85            	popw	x
 856  019d 81            	ret
 912                     ; 195 bool clear_button_event(u8 button_index,bool is_long)
 912                     ; 196 {
 913                     	switch	.text
 ...
 921                     ; 197 	bool out=button_pressed_event[button_index][is_long];
 923  01a0 9e            	ld	a,xh
 924  01a1 5f            	clrw	x
 925  01a2 97            	ld	xl,a
 926  01a3 58            	sllw	x
 ...
 936                     ; 198 	button_pressed_event[button_index][is_long]=0;
...
1030  01db 84            	pop	a
1031  01dc 81            	ret
1067                     ; 213 bool is_button_down(u8 index)
1067                     ; 214 {
1068                     	switch	.text
1069  01dd               _is_button_down:
1073                     ; 215 	switch(index)
1076                     ; 219 		case 2:{ return !GPIO_ReadInputPin(GPIOD, GPIO_PIN_1); break; }//SWIM IO input
1078  01dd 4d            	tnz	a
...
1085  01e8               L133:
1086                     ; 217 		case 0:{ return !GPIO_ReadInputPin(GPIOD, GPIO_PIN_5); break; }//left button
...
1102  01fa 81            	ret
1103  01fb               L333:
1104                     ; 218 		case 1:{ return !GPIO_ReadInputPin(GPIOD, GPIO_PIN_6); break; }//right button
...
1121  020d 81            	ret
1122  020e               L533:
1123                     ; 219 		case 2:{ return !GPIO_ReadInputPin(GPIOD, GPIO_PIN_1); break; }//SWIM IO input
...
1144  0221 4f            	clr	a
1147  0222 81            	ret
1197                     ; 224 void update_audio()
1197                     ; 225 {
1198                     	switch	.text
1199  0223               _update_audio:
1201  0223 5203          	subw	sp,#3
1202       00000003      OFST:	set	3
1205                     ; 227 	reading=ADC1->DRL;//only get 8 least significant bits of the 10 available, assuming reading is not changing fast enough or near enough the 1/4-full-scale point to matter
1207  0225 c65405        	ld	a,21509
1208  0228 6b03          	ld	(OFST+0,sp),a
1210                     ; 228 	ADC1_ClearFlag(ADC1_FLAG_EOC);
1212  022a a680          	ld	a,#128
1213  022c cd0000        	call	_ADC1_ClearFlag
1215                     ; 229 	audio_measurement_count++;
1217  022f 3c08          	inc	_audio_measurement_count
1218                     ; 230 	audio_running_mean+=reading;
...
1228                     ; 231 	reading_residual=reading>audio_mean?reading-audio_mean:audio_mean-reading;
...
1242                     ; 232 	audio_running_std+=reading_residual;
...
1430                     ; 258 	u8 sleep_amount=PWM_MAX_PERIOD;//max sleep duration before wrap-over occurs
1432  0297 a6fa          	ld	a,#250
1433  0299 6b04          	ld	(OFST+0,sp),a
1435                     ; 259 	bool buffer_index=pwm_state&0x01;//primary vs redundant side to pull data from
1437  029b b607          	ld	a,_pwm_state
1438  029d a401          	and	a,#1
1439  029f 6b03          	ld	(OFST-1,sp),a
1441                     ; 261 	TIM2->SR1&=~TIM2_SR1_CC1IF;//reset interrupt
1443  02a1 72135304      	bres	21252,#1
1444                     ; 263 	if(pwm_sleep_remaining==0)
1446  02a5 be04          	ldw	x,_pwm_sleep_remaining
1447  02a7 2657          	jrne	L744
1448                     ; 265 		set_matrix_high_z();
1450  02a9 cd04b7        	call	_set_matrix_high_z
1452                     ; 266 		if(pwm_led_index<pwm_led_count[buffer_index])
...
1460                     ; 268 			set_led(pwm_brightness[pwm_led_index][0][buffer_index]);//turn on the next LED
...
1475                     ; 269 			pwm_sleep_remaining=0x00FF&(pwm_brightness[pwm_led_index][1][buffer_index]+15);//set how long to sleep in this state //+15 works ok here as magic number to correct timing errors from interrupts
...
1497                     ; 270 			pwm_led_index++;//prepare state machine to go to the next led later
1499  02e5 3c06          	inc	_pwm_led_index
1501  02e7 2017          	jra	L744
1502  02e9               L154:
1503                     ; 272 			pwm_led_index=0;//reset state machine to 0
1505  02e9 3f06          	clr	_pwm_led_index
1506                     ; 273 			pwm_sleep_remaining=pwm_sleep[buffer_index];
...
1514                     ; 274 			if(pwm_state&0x02) pwm_state^=0x03;//if flag to swap A/B is set, then clear the flag and swap sides
...
1524  0300               L744:
1525                     ; 277 	sleep_amount=pwm_sleep_remaining<sleep_amount?pwm_sleep_remaining:sleep_amount;
...
1541                     ; 278 	pwm_sleep_remaining-=sleep_amount;
...
1551                     ; 280 	TIM2->CCR1L = ( (TIM2->CCR1L) + sleep_amount )%PWM_MAX_PERIOD;//set wakeup alarm relative to current time
...
1577  0346 80            	iret
1642                     ; 283 void flush_leds(u8 led_count)
1642                     ; 284 {
1644                     	switch	.text
...
1652                     ; 285 	u8 led_read_index=0,led_write_index=0;
1656  034a 0f02          	clr	(OFST-1,sp)
1659  034c               L515:
1660                     ; 287 	while(pwm_state&0x02){}//wait for volatile flag to clear (if still raised from the previous call)
1662  034c b607          	ld	a,_pwm_state
1663  034e a502          	bcp	a,#2
1664  0350 26fa          	jrne	L515
1665                     ; 288 	buffer_index=0x01^(pwm_state&0x01);//need to wait for above flag to be cleared before evaluating this
1667  0352 b607          	ld	a,_pwm_state
1668  0354 a401          	and	a,#1
1669  0356 a801          	xor	a,#1
1670  0358 6b01          	ld	(OFST-2,sp),a
1672                     ; 289 	pwm_sleep[buffer_index]=led_count<<8;//prepare the max value of sleep, and subtract from it for each LED illuminated
...
1684                     ; 291 	for(led_read_index=0;(led_read_index<LED_COUNT && led_write_index<led_count);led_read_index++)
1686  036b 0f03          	clr	(OFST+0,sp)
1689  036d 205d          	jra	L525
1690  036f               L125:
1691                     ; 293 		if(pwm_brightness_buffer[led_read_index]>4)//min brightness, below this value instaiblity occurs magic number to avoid interrupt timing error
...
1699                     ; 295 			pwm_brightness[led_write_index][0][buffer_index]=led_read_index;
...
1713                     ; 296 			pwm_brightness[led_write_index][1][buffer_index]=pwm_brightness_buffer[led_read_index];
...
1730                     ; 297 			led_write_index++;
1732  03a2 0c02          	inc	(OFST-1,sp)
1734                     ; 298 			pwm_sleep[buffer_index]-=pwm_brightness_buffer[led_read_index];
...
1753  03c4               L135:
1754                     ; 300 		pwm_brightness_buffer[led_read_index]=0;//clean up for next use
1756  03c4 7b03          	ld	a,(OFST+0,sp)
1757  03c6 5f            	clrw	x
1758  03c7 97            	ld	xl,a
1759  03c8 6fc0          	clr	(_pwm_brightness_buffer,x)
1760                     ; 291 	for(led_read_index=0;(led_read_index<LED_COUNT && led_write_index<led_count);led_read_index++)
...
1774  03d8               L335:
1775                     ; 302 	pwm_led_count[buffer_index]=led_write_index;//save the led count for the volatile pwm rutine state machine.
1777  03d8 7b01          	ld	a,(OFST-2,sp)
1778  03da 5f            	clrw	x
1779  03db 97            	ld	xl,a
1780  03dc 7b02          	ld	a,(OFST-1,sp)
1781  03de e70e          	ld	(_pwm_led_count,x),a
1782                     ; 305 	pwm_state|=0x02;//raise flag that data is ready for volatile pwm process to pick up and use
1784  03e0 72120007      	bset	_pwm_state,#1
1785                     ; 306 }
1788  03e4 5b04          	addw	sp,#4
1789  03e6 81            	ret
1878                     ; 308 void set_hue(u8 index,u16 color,u8 brightness)
1878                     ; 309 {
1879                     	switch	.text
1880  03e7               _set_hue:
1882  03e7 88            	push	a
...
1907                     ; 312 	residual=(u8)(residual*brightness/0x2AAB);
...
1920                     ; 313 	switch(color/(0x2AAB))//0xFFFF/6
...
2060  047b 84            	pop	a
2061                     ; 348 	set_rgb(index,1,green);
...
2085  0496 81            	ret
2138                     ; 352 void set_rgb(u8 index,u8 color,u8 brightness)
2138                     ; 353 {
2139                     	switch	.text
2140  0497               _set_rgb:
2142  0497 89            	pushw	x
2143       00000000      OFST:	set	0
2146                     ; 354 	pwm_brightness_buffer[index*3+color]=brightness;
...
2163  04a8 85            	popw	x
2164  04a9 81            	ret
2208                     ; 357 void set_white(u8 index,u8 brightness)
2208                     ; 358 {
2209                     	switch	.text
2210  04aa               _set_white:
2212  04aa 89            	pushw	x
2213       00000000      OFST:	set	0
2216                     ; 359 	pwm_brightness_buffer[31+index]=brightness;
...
2226  04b2 85            	popw	x
2227  04b3 81            	ret
2262                     ; 363 void set_debug(u8 brightness)
2262                     ; 364 {
2263                     	switch	.text
2264  04b4               _set_debug:
2268                     ; 365 	pwm_brightness_buffer[30]=brightness;
2270  04b4 b7de          	ld	_pwm_brightness_buffer+30,a
2271                     ; 366 }
2274  04b6 81            	ret
2299                     ; 368 void set_matrix_high_z()
2299                     ; 369 {
2300                     	switch	.text
2301  04b7               _set_matrix_high_z:
2305                     ; 372 		GPIO_Init(GPIOC, GPIO_PIN_7 | GPIO_PIN_6 | GPIO_PIN_5 | GPIO_PIN_4 | GPIO_PIN_3, GPIO_MODE_IN_FL_NO_IT);//2, 3, 4, 5, 6
2307  04b7 4b00          	push	#0
2308  04b9 4bf8          	push	#248
2309  04bb ae500a        	ldw	x,#20490
2310  04be cd0000        	call	_GPIO_Init
2312  04c1 85            	popw	x
2313                     ; 373 		GPIO_Init(GPIOD, GPIO_PIN_3 | GPIO_PIN_2, GPIO_MODE_IN_FL_NO_IT);
2315  04c2 4b00          	push	#0
2316  04c4 4b0c          	push	#12
2317  04c6 ae500f        	ldw	x,#20495
2318  04c9 cd0000        	call	_GPIO_Init
2320  04cc 85            	popw	x
2321                     ; 374 		GPIO_Init(GPIOA, GPIO_PIN_3 , GPIO_MODE_IN_FL_NO_IT);
...
2329                     ; 378 }
2332  04d8 81            	ret
2356                     ; 381 u8 get_audio_sample()
2356                     ; 382 {
2357                     	switch	.text
2358  04d9               _get_audio_sample:
2362                     ; 388 	return 0;//revision 1 hw misrouted this connection, made it un-readable
2364  04d9 4f            	clr	a
2367  04da 81            	ret
2370                     	switch	.const
...
2456  005a 05            	dc.b	5
2457  005b 06            	dc.b	6
2668                     ; 392 void set_led(u8 led_index)
2668                     ; 393 {
2669                     	switch	.text
2670  04db               _set_led:
2672  04db 88            	push	a
...
2677                     ; 397 	const u8 led_lookup[LED_COUNT][2]={//[0] is HIGH mat, [1] is LOW mat
2677                     ; 398 		{0,1},{0,2},{1,2},//LED7  RGB
2677                     ; 399 		{1,0},{2,0},{2,1},//LED3  RGB
2677                     ; 400 		{5,0},{5,1},{5,2},//LED1  RGB
2677                     ; 401 		{6,0},{6,1},{6,2},//LED20 RGB
2677                     ; 402 		{6,5},{6,4},{5,4},//LED22 RGB
2677                     ; 403 		{4,3},{5,3},{6,3},//LED23 RGB
2677                     ; 404 		{3,4},{3,5},{3,6},//LED21 RGB
2677                     ; 405 		{0,5},{0,6},{1,6},//LED19 RGB
2677                     ; 406 		{0,4},{1,4},{2,4},//LED18 RGB
2677                     ; 407 		{0,3},{1,3},{2,3},//LED2  RGB
2677                     ; 408 		{7,7},//debug; GND is tied low, no charlieplexing involved
2677                     ; 409 		{3,0},//LED6
2677                     ; 410 		{3,1},//LED4
2677                     ; 411 		{3,2},//LED5
2677                     ; 412 		{4,0},//LED14
...
2685                     ; 422 	bool is_high=0;
2687  04eb 0f5c          	clr	(OFST+0,sp)
2689  04ed               L1011:
2690                     ; 426 		switch(led_lookup[led_index][!is_high])
...
2880  05a9 81            	ret
2904                     ; 470 bool is_space_sao()
2904                     ; 471 {
2905                     	switch	.text
2906  05aa               _is_space_sao:
2910                     ; 472 	return 1;//TODO: implement EEPROM read
2912  05aa a601          	ld	a,#1
2915  05ac 81            	ret
3097                     	xdef	f_TIM2_CapComp_IRQ_Handler
3098                     	xdef	f_TIM2_UPD_OVF_IRQHandler
3099                     	switch	.ubsct
3100  0000               _audio_running_std:
3101  0000 0000          	ds.b	2
3102                     	xdef	_audio_running_std
3103  0002               _audio_running_mean:
3104  0002 0000          	ds.b	2
3105                     	xdef	_audio_running_mean
3106  0004               _audio_std:
3107  0004 00            	ds.b	1
3108                     	xdef	_audio_std
3109  0005               _audio_mean:
3110  0005 00            	ds.b	1
3111                     	xdef	_audio_mean
3112                     	xdef	_audio_measurement_count
3113  0006               _button_pressed_event:
3114  0006 00000000      	ds.b	4
3115                     	xdef	_button_pressed_event
3116  000a               _button_start_ms:
3117  000a 00000000      	ds.b	4
3118                     	xdef	_button_start_ms
3119                     	xdef	_pwm_state
3120                     	xdef	_pwm_led_index
3121                     	xdef	_pwm_sleep_remaining
3122  000e               _pwm_led_count:
3123  000e 0000          	ds.b	2
3124                     	xdef	_pwm_led_count
3125  0010               _pwm_sleep:
3126  0010 00000000      	ds.b	4
3127                     	xdef	_pwm_sleep
3128  0014               _pwm_brightness:
3129  0014 000000000000  	ds.b	172
3130                     	xdef	_pwm_brightness
3131                     	xdef	_PWM_MAX_PERIOD
3132  00c0               _pwm_brightness_buffer:
3133  00c0 000000000000  	ds.b	43
3134                     	xdef	_pwm_brightness_buffer
3135                     	xdef	_api_counter
3136                     	xdef	_hw_revision
3137                     	xdef	_set_millis
3138                     	xdef	_get_audio_level
3139                     	xdef	_get_random
3140                     	xdef	_is_space_sao
3141                     	xdef	_update_audio
3142                     	xdef	_is_button_down
3143                     	xdef	_clear_button_events
3144                     	xdef	_clear_button_event
3145                     	xdef	_get_button_event
3146                     	xdef	_update_buttons
3147                     	xdef	_is_sleep_valid
3148                     	xdef	_is_developer_valid
3149                     	xdef	_set_hue
3150                     	xdef	_flush_leds
3151                     	xdef	_set_led
3152                     	xdef	_get_audio_sample
3153                     	xdef	_set_debug
3154                     	xdef	_set_white
3155                     	xdef	_set_rgb
3156                     	xdef	_set_matrix_high_z
3157                     	xdef	_millis
3158                     	xdef	_setup_main
3159                     	xdef	_is_application_valid
3160                     	xdef	_setup_serial
3161                     	xref	_UART1_Cmd
3162                     	xref	_UART1_Init
3163                     	xref	_UART1_DeInit
3164                     	xref	_GPIO_ReadInputPin
3165                     	xref	_GPIO_Init
3166                     	xref	_ADC1_ClearFlag
3167                     	xref	_ADC1_StartConversion
3168                     	xref.b	c_lreg
3169                     	xref.b	c_x
3170                     	xref.b	c_y
3190                     	xref	c_xymov
